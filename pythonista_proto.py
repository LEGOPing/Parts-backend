...
# ⚠ 请勿填入任何零件作为"示例"保留，调试完务必清空，否则会覆盖价格库！
MANUAL_PARTS = []         # 例：[('3001','85')]  ← 调试完立即清空回 []
...
def _run_closed_loop():
    ...
    # 回写 Gitee（先校验，防止用少量数据意外覆盖远端完整价格库）
    remote_count = 0
    try:
        remote_records, _ = load_existing_price()
        remote_count = len(remote_records)
    except Exception as e:
        log('  无法读取远端现有价格库（%s），跳过安全校验' % e)
    ...
    if remote_count > 0 and local_count < remote_count:
        ...
        log('  本地副本已保存在 result.json，请检查 MAX_FETCH_PER_RUN / MANUAL_PARTS 设置')
        log('  若确认要推送，请先手动备份远端仓库并在代码中注释掉本段保护逻辑')
        return
...
def _run_manual():
    """手工自测模式：直接抓 MANUAL_PARTS，写 result.json，不推送 Gitee（仅限本地调试！）。"""
    global _results
    todo = []
    for p, c in MANUAL_PARTS:
        ...
        todo.append((p, c, key))
    _results = []
    log('⚠ 手动自测模式（本地 result.json 测试，不走 Supabase，不推 Gitee），待抓 %d 组' % len(todo))
    ok = fail = 0
    for idx, (part, bl_cid, key) in enumerate(todo, start=1):
        rec = _fetch_one(_webview, part, bl_cid, idx, len(todo))
        if rec:
            ...
            fail += 1
    save_local(_results)
    log('=== 手动模式完成：成功 %d / 跳过 %d（仅限本地，未推送 Gitee）===' % (ok, fail))
