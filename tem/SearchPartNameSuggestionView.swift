...
// 搜索页面的零件名称联想器视图
struct SearchPartNameSuggestionView: View {
    ...
    var tableType: PartNameSuggestion.TableType = .part // 默认使用Part表
        ...
        VStack(alignment: .leading, spacing: 4) {
                ...
                TextField("请输入零件名称", text: $partNameInput)
                        ...
                        GeometryReader { geometry in
                            Color.clear
                                .onAppear {
                                    ...
                                    if inputBoxFrame != nil {
                                        inputBoxFrame = geometry.frame(in: .named("GeometryReader"))
                                        print("零件名称输入框坐标: \(inputBoxFrame!)")
                                    }
                                }
                                .onChange(of: geometry.frame(in: .named("GeometryReader"))) { oldValue, newValue in
                                    if inputBoxFrame != nil {
                                        inputBoxFrame = newValue
                                        print("零件名称输入框坐标已更新: \(inputBoxFrame!)")
                                    }
                                }
                        }
                    )
                    .onChange(of: partNameInput) {
                        ...
                        nameSuggestionTimer = Timer.scheduledTimer(withTimeInterval: 0.5, repeats: false) { _ in
                            // 获取之前输入的单词（使用智能分词）
                            let previousWords = SearchPartNameSuggestionView.smartTokenize(newValue)
                ...
                PartNameSuggestion.getSuggestionsAsync(for: effectiveCurrentWord, wordIndex: effectiveWordIndex, previousWords: previousWords, tableType: tableType) { suggestions in
                    DispatchQueue.main.async {
                        ...
                        if partNameInput == currentInput {
                            ...
                            showNameSuggestions = !suggestions.isEmpty
                        }
                    }
                }
                            } else {
                    ...
                    .onTapGesture {
                        ...
                        // 获取之前输入的单词（使用智能分词）
                        let previousWords = SearchPartNameSuggestionView.smartTokenize(partNameInput)
            ...
            if showNameSuggestions && !nameSuggestions.isEmpty {
                    ...
                    SuggestionPopupView(
                        ...
                        onSelect: { suggestion in
                            ...
                            // 更新输入框内容，添加空格以便输入下一个单词
                                    ...
                                    // 通知外部partName已变化
                                    onPartNameChange?(partNameInput.trimmingCharacters(in: .whitespaces))
                            ...
                            DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
                                ...
                                // 获取之前输入的单词（使用智能分词）
                                let previousWords = SearchPartNameSuggestionView.smartTokenize(newText)
                ...
                if !currentWord.isEmpty {
                    ...
                    .frame(maxWidth: 500, maxHeight: 400) // 限制最大大小
        ...
        var body: some View {
            VStack(spacing: 10) {
                ...
                ScrollView(.vertical, showsIndicators: true) {
                    VStack(spacing: 15) {
                        ForEach(groupedSuggestions, id: \.key) { group in
                            VStack(alignment: .leading, spacing: 8) {
                                ...
                                ScrollView(.horizontal, showsIndicators: false) {
                                    HStack(spacing: 8) {
                                        ForEach(group.value, id: \.self) { suggestion in
                                            Button(action: {
                                                onSelect(suggestion)
                                            }) {
                                                Text(suggestion)
