...
struct AsyncImageLoader: View {
    ...
    var body: some View {
        ...
        .onAppear {
            print("AsyncImageLoader 视图出现，开始加载图片")
            loadImage()
        }
    }
    ...
    private func loadImage() {
        // 从part对象中获取零件型号和颜色ID
        guard let partNum = part.value(forKey: "part_num") as? String, !partNum.isEmpty, 
              let colorId = part.value(forKey: "color_id") as? Int32 else {
            print("零件型号或颜色ID为空，不加载图片")
            isLoading = false
            image = nil
            return
        }
        ...
        // 首先尝试从本地缓存加载图片
        let cachedImage = loadCachedImage(partNum: partNum, colorId: colorId)
        if let cachedImage = cachedImage {
            ...
            if let cachedUrl = ImageUrlCache.get(partNum: partNum, colorId: colorId) {
                ...
                fetchImageUrl(partNum: partNum, colorId: colorId) { urlString in
                    // 通知父视图图片URL已经更新
                            ...
                            if error != nil {
                                ...
                                if httpResponse.statusCode != 200 {
                                    ...
                                    if let loadedImage = UIImage(data: data) {
                                        ...
                                        // 保存图片到本地缓存
                                        self.saveImageToCache(image: loadedImage, partNum: partNum, colorId: colorId)
                                    } else {
                                        print("图片数据无效")
                                        print("URL: \(urlString)")
                                        print("零件型号: \(partNum)")
    ...
    private func fetchImageUrl(partNum: String, colorId: Int32, completion: @escaping (String?) -> Void) {
        // 检查缓存是否有效
        if let cachedUrl = ImageUrlCache.get(partNum: partNum, colorId: colorId) {
            print("使用缓存的图片URL: \(cachedUrl)")
    ...
    private func loadCachedImage(partNum: String, colorId: Int32) -> UIImage? {
        // 生成缓存文件名
    ...
    private func saveImageToCache(image: UIImage, partNum: String, colorId: Int32) {
        // 生成缓存文件名
        let cacheFileName = "\(partNum)_\(colorId).jpg"
