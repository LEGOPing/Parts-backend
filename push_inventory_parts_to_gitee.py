...
背景：
    ...
    再切分为多个 <4MB 的分片文件（考虑 Gitee 写操作 429 限流）推送到 Gitee，
    前端依次下载各分片合并后导入，即可完整读取。分片序号从 1 开始（inventory_parts_1.csv ...）。
...
# 分片配置：每个分片小于 4MB（Gitee contents API 截断阈值约 10MiB，同时考虑写操作 429 限流，前端按钮同规格）
MAX_SHARD_BYTES = 4 * 1024 * 1024

# 分片文件名模板（与前端 frontend/js/api.js 保持一致，序号从 1 开始）
...
def gitee_request(method, path, payload=None, max_retries=5):
    """调用 Gitee API，返回 (status_code, response_json)。
    写操作遇 429 限流或网络异常时按指数退避重试（1s→2s→4s→...→60s）。"""
    import time
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            log(f"HTTP 429 限流，{int(delay)} 秒后第 {attempt + 1} 次重试...")
            time.sleep(delay)
            ...
            log(f"网络异常({e})，{int(delay)} 秒后第 {attempt + 1} 次重试...")
            time.sleep(delay)
            delay = min(delay * 2, 60.0)
...
def dedup_and_split(csv_path):
    """按 (part_num, color_id) 去重，再切分为 <4MB 的分片（每个分片都带表头）"""
    ...
    # 逐行切分，保证每个分片（含表头）严格 < MAX_SHARD_BYTES
...
def main():
    ...
    try:
        ...
        manifest = {
            "count": len(shards),
            "files": [f"{SHARD_BASE}{i + 1}{SHARD_SUFFIX}" for i in range(len(shards))],
            "rows": len(dedup_rows),
        ...
        if args.dry_run:
            log("【模拟模式】不会实际推送")
            for i, s in enumerate(shards):
                fname = f"{SHARD_BASE}{i + 1}{SHARD_SUFFIX}"
                log(f"  将推送 {fname} ({len(s.encode('utf-8'))} 字节)")
            log(f"  将推送 {MANIFEST}")
            return

        # 3. 推送分片（PUT 更新 / POST 创建，片间间隔 2.5s 规避 429 限流）
        for i, s in enumerate(shards):
            fname = f"{SHARD_BASE}{i + 1}{SHARD_SUFFIX}"
        ...
        for i in range(len(shards)):
            log(f"  https://gitee.com/legoping/parts-rb/raw/main/{SHARD_BASE}{i + 1}{SHARD_SUFFIX}")
    finally:
